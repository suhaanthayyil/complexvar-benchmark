...
### External validation framework using orthogonal data sources

To assess generalization beyond the SKEMPI training set, we developed an external validation pipeline that combines deep mutational scanning (DMS) data from MaveDB with binary protein interaction data from the HuRI network. This approach provides validation on an independent data source with different characteristics: DMS experiments measure functional effects through high-throughput assays rather than binding affinity measurements, and the variant distribution reflects comprehensive saturation mutagenesis rather than targeted single mutations.

The validation pipeline identifies genes that have published DMS maps in MaveDB, documented binary interactions in the HuRI network, and high-confidence AlphaFold2 complex structures in the Burke et al. dataset. For each intersecting gene, variants from the DMS score set are mapped to protein-protein interfaces (defined as residues within 10 angstroms of the partner chain) and labeled using a percentile-based scheme where the bottom 20% of functional scores are classified as disruptive. The trained ComplexVar model is then evaluated on these interface-proximal variants to compute an external validation AUROC.

Based on the expected overlap between MaveDB (approximately 300 human protein score sets), HuRI (53,000 binary interactions covering 8,000 proteins), and Burke structures (3,137 high-confidence complexes), we anticipate 15-25 genes will meet all criteria, yielding 200-400 interface-proximal variants for external validation. The full pipeline implementation is provided in scripts/external_validation.py. This framework enables ongoing validation as new DMS datasets are published and demonstrates a reproducible methodology for independent performance assessment using orthogonal functional data.
