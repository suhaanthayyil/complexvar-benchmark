...
Missense variants at protein-protein interfaces can disrupt critical molecular interactions, yet most computational pathogenicity predictors rely on sequence information or monomer structures alone, ignoring the structural context of the protein complex. Here we present ComplexVar, a graph attention network trained on residue contact graphs derived from experimentally determined and computationally predicted protein complex structures. We benchmark five models on SKEMPI 2.0 binding affinity data: a sequence-only baseline using ESM2 embeddings, a monomer graph neural network, a complex graph neural network incorporating inter-chain contacts, and two engineered structural baselines (logistic regression and histogram-based gradient boosting). On interface-proximal variants (within 8 angstroms of the partner chain), the complex GNN (AUROC 0.702) significantly improves over the monomer GNN (0.682, delta +0.020, p = 0.022) and the sequence baseline (0.641, delta +0.061, p = 0.003). However, engineered structural baselines achieve higher absolute performance (logistic 0.740, HGB 0.750), indicating that the GNN architecture rather than the complex context concept is the current limiting factor. On the SKEMPI regression task, the complex model yields the best rank correlation (Spearman rho = 0.462) compared with the monomer model (0.426) and the sequence baseline (0.336). Computational scoring of 277 interface-proximal ClinVar variants of uncertain significance is provided as a supplementary resource for community use. Five-fold protein-grouped cross-validation reveals that this advantage does not consistently replicate across splits: the monomer GNN outperforms the complex GNN in all five folds (mean delta = -0.020). Our results suggest that while complex context provides a measurable signal on favorable splits, the benefit is not robust, and architecture and feature engineering choices remain the primary determinants of overall performance.
...
The complex GNN achieved an overall AUROC of 0.754 on the full test set (Table 1), compared with 0.747 for the monomer GNN and 0.646 for the sequence-only baseline. Both GNN models substantially outperformed the sequence baseline, confirming the value of structural information. The improvement of the complex GNN over the monomer GNN was statistically significant (paired bootstrap delta AUROC = +0.018, p = 0.024; DeLong z = 2.03, p = 0.042).

Our primary evaluation focuses on interface-proximal variants (those within 8 angstroms of the partner chain), where complex context is most relevant. On a common evaluation set of 770 interface-proximal variants scored by all five models, the complex GNN achieved an AUROC of 0.702 (95% CI [0.665, 0.739]), compared with the monomer GNN at 0.682 (95% CI [0.644, 0.719]) and the sequence baseline at 0.641 (95% CI [0.602, 0.681]) (Table 2). The improvement from monomer to complex context was statistically significant (delta = +0.020, p = 0.022), supporting the hypothesis that inter-chain contacts provide additional discriminative signal at interfaces. The complex GNN also significantly outperformed the sequence baseline (delta = +0.061, p = 0.003).
...
The engineered structural baselines (logistic regression and histogram-based gradient boosting) achieved overall AUROCs of 0.788 and 0.792, respectively, substantially outperforming both GNN models on this benchmark (Table 1). On the interface-proximal subset, the gap persists: logistic regression achieves 0.740 and HGB achieves 0.750, compared with 0.702 for the complex GNN (Table 2). A hybrid model combining GNN embeddings with tabular structural features via HGB achieved the best overall MCC (0.429) and an AUROC of 0.781, suggesting that learned graph representations and engineered features capture complementary information.
...
### Interface-proximal comparison across all models

To directly test whether complex structural context provides an advantage at protein-protein interfaces, we compared all five models on the interface-proximal subset of the test set (n = 770 variants within 8 angstroms of the partner chain; Table 2). On this subset, the complex GNN (AUROC 0.702) significantly outperformed the monomer GNN (0.682, paired bootstrap delta = +0.020, p = 0.022) and the sequence baseline (0.641, delta = +0.061, p = 0.003). However, the engineered structural baselines achieved higher absolute AUROCs: logistic regression (0.740) and histogram-based gradient boosting (0.750). The complex GNN's AUROC was significantly lower than both structural baselines (delta = -0.038 vs logistic, p = 0.99; delta = -0.048 vs HGB, p = 1.00).

These results confirm that complex context improves GNN-based predictions at interfaces (complex GNN vs monomer GNN), but also show that well-engineered structural feature sets can capture much of this information without explicit graph-based learning. The structural baselines use curated features including interface distance, burial proxies, and solvent exposure that directly encode interface geometry, partially explaining their strong performance on this subset.

**Table 2.** Interface-proximal model comparison (variants within 8 angstroms of partner chain, n = 770).

| Model | Interface AUROC | 95% CI | Delta vs Complex GNN | p-value |
|---|---|---|---|---|
...
### Computational scoring of ClinVar VUS

We applied the trained complex GNN to score 277 interface-proximal VUS from ClinVar that mapped onto high-confidence protein complexes (Supplementary Table S4). These scores have not been experimentally validated and are provided as a supplementary resource for community investigation; details of the scoring and disease category distribution are in the Supplementary Information.

## Discussion

On the held-out test set, the complex GNN shows a statistically significant but modest improvement over the monomer GNN on interface-proximal variants (delta AUROC = +0.020, p = 0.022), with the advantage increasing monotonically with interface proximity, reaching +0.054 at distances less than 5 angstroms. However, five-fold protein-grouped cross-validation (Supplementary Table S9) tells a different story: the monomer GNN consistently outperforms the complex GNN across all folds (mean delta = -0.020, 95% CI [-0.028, -0.012]). This discrepancy between held-out test and cross-validation results is an important caveat that tempers the interpretation of complex context benefits.

Several observations merit discussion. First, the improvement from monomer to complex context, while statistically significant and consistent, is smaller in magnitude than the improvement from sequence to structure. This suggests that the bulk of the structural signal is already captured by monomer features such as solvent accessibility, secondary structure, and local packing, and that inter-chain contacts provide an incremental contribution most impactful for specific interface-proximal subsets. The ablation studies confirm this hierarchy: removing all structural features degrades performance far more (delta AUROC = -0.165) than removing inter-chain edges alone (delta = -0.004).

Second, the engineered structural baselines (logistic regression AUROC 0.740, HGB 0.750 on interface-proximal) significantly outperform both GNN models on interface-proximal variants (Table 2). This indicates that complex structural context improves GNN performance over the monomer GNN baseline, but the GNN architecture itself is the current limiting factor relative to well-tuned feature-based approaches. The structural baselines use curated features that directly encode interface geometry (distance, burial, solvent exposure), and their strong performance suggests that much of the predictive signal at interfaces can be captured without explicit graph-based learning. The hybrid model combining GNN embeddings with tabular features achieved the highest MCC (0.429), confirming that learned graph representations and engineered features capture complementary information.

### Limitations

This study has several important limitations that should be considered when interpreting the results.

**Modest effect size.** The improvement from complex to monomer context (delta AUROC = +0.020) is statistically significant but small in absolute terms, with the 95% confidence interval lower bound near zero ([+0.001, +0.040]). This means the practical benefit of adding complex context is incremental rather than transformative. Structural baselines with curated features outperform both GNN models, suggesting that architecture and feature engineering choices have a larger impact on performance than the inclusion of inter-chain contacts.

**Single benchmark dataset and CV-test discrepancy.** All training and held-out evaluation is based on SKEMPI 2.0, which covers only 323 protein complexes and may not represent the diversity of protein-protein interfaces in the human proteome. Five-fold protein-grouped cross-validation (Supplementary Table S9) reveals that the monomer GNN consistently outperforms the complex GNN across all folds (mean delta = -0.020, 95% CI [-0.028, -0.012]), contrasting with the held-out test result where the complex GNN achieves higher AUROC. This discrepancy suggests the held-out test advantage may be partially due to the specific train-test split rather than a robust advantage of complex context. The complex GNN's greater parameter count may make it more sensitive to training conditions.

**No external validation.** The reported results are based solely on internal held-out test evaluation on SKEMPI 2.0. External validation using independent datasets such as MaveDB deep mutational scanning data intersected with HuRI binary interaction networks has not been performed. Until such validation is completed, the generalizability of these results to new protein complexes remains uncertain.

**Structural baseline competitiveness.** On the interface-proximal evaluation set, engineered structural baselines (logistic regression and HGB) achieve significantly higher AUROCs than the complex GNN (Table 2). This suggests that the current GNN architecture, rather than the concept of complex structural context, is the primary bottleneck. Future work should explore whether more expressive GNN architectures or pre-trained structural encoders can close this gap.

**Predicted structure uncertainties.** The AlphaFold2-predicted complex structures used for ClinVar scoring carry inherent uncertainties, particularly for complexes with lower pDockQ scores. We restricted to high-confidence predictions (pDockQ > 0.5), but prediction errors in interface geometry could still affect variant scoring accuracy.

**Unvalidated VUS predictions.** The ClinVar VUS scores have not been experimentally validated and should be treated only as computational prioritization aids. Any clinical use would require independent experimental confirmation.
...
On the held-out test set, incorporating protein complex structural context into graph-based variant effect prediction provides a statistically significant but modest advantage for interface-proximal variants (delta AUROC = +0.020 over monomer GNN, p = 0.022). However, this advantage does not replicate in five-fold cross-validation, where the monomer GNN consistently outperforms the complex GNN (mean delta = -0.020). Furthermore, the current GNN architecture does not surpass well-engineered structural feature baselines on this benchmark. These results indicate that complex structural context provides a detectable but non-robust signal, and that architecture and feature engineering improvements are needed before complex-aware GNNs can serve as primary clinical variant interpretation tools. The ComplexVar benchmark framework, including data processing pipelines, trained models, and evaluation code, is publicly available to enable reproduction and extension of this work.
