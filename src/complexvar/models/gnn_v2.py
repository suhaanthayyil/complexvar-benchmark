    ...
    class ComplexVarGAT(nn.Module):
        ...
        def _get_mutant_embedding(self, x, data):
            ...
            if not isinstance(mutant_idx, torch.Tensor):
                mutant_idx = torch.tensor(
                    [mutant_idx], dtype=torch.long, device=x.device,
                )
            if mutant_idx.ndim == 0:
                mutant_idx = mutant_idx.unsqueeze(0)

            # Offset local per-graph indices to global positions
            if hasattr(data, "ptr") and len(mutant_idx) > 1:
                offsets = data.ptr[:-1].to(mutant_idx.device)
